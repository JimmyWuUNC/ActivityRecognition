function [finalTable] = featureExtractionTestFileWise(varargin)
...
for masterVariable=1:nargin;%Goes over all the files
    ...
    samplingRate=25; %based on experimentation
    timeExercisedInSeconds = zeroes(masterVariable,1);
    table_x1 = [];
    table_y1 = [];
    table_z1 = [];
    table_accel1 = [];
    ...
    [rows,~] = size(table_x1);
    timeExercisedInSeconds(masterVariable,1)= rows / samplingRate;
    ...
    finalTable1=table(table_xMean,table_xStDev,table_yMean,table_yStDev,table_zMean,table_zStDev,table_accelMean,table_accelStDev);
